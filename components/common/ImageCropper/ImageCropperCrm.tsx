...
import { getCroppedImg, getRotatedImage } from "./CanvsUtlis";
import { Controls, CropContainer, SliderContainer } from "./style";
import {
  Box,
  Button,
  Divider,
...
import { LeadsListing } from "@/hooks/LeadsReducer";
import {
  getAccountsPersonaldetails,
  getContactsPersonaldetails,
  getDealsPersonaldetails,
...
const ImageCropperCrm = (props) => {
  ...
  const SaveImage = async (Image: any) => {
    if (endPoints) {
      ...
      if (type === "leads") {
        ...
        url = `leads/profile-upload`;
      } else if (type === "contacts") {
        formData.append("file", Image);
        formData.append("contact_id", contact_id);
        url = `contact/profile-upload`;
      } else if (type === "deals") {
        formData.append("file", Image);
        formData.append("deal_id", deal_id);
        url = `deal/profile-upload`;
      }
      ...
      if (response.status == 200) {
        if (endPoints) dispatch(getLeadsAllFieldDataAndUpdateStates());
        if (endPoints)
          if (type === "leads") {
            dispatch(getLeadsPersonaldetails({ lead_id }));
          } else if (type === "contacts") {
            dispatch(getContactsPersonaldetails({ contact_id }));
          } else if (type === "deals") {
            dispatch(getDealsPersonaldetails({ deal_id }));
          }
  ...
  //       );
  //       if (response.status == 200) {
  //         // if (endPoints) dispatch(LeadsListing());
    ...
    <>
            ...
            <CropContainer>
              {editDefaultImage ? (
                ...
                <>
                  <Cropper
                    cropShape={
                      type == "banner" || type == "square" ? "rect" : "round"
                    }
                    aspect={
                      type == "banner"
                        ? 9 / 2
      ...
      {icon ? (
        ...
        <FloatingUpdateIconContainer
          sx={{
            bottom: uploadIcon && "-6px",
            right: uploadIcon && "-4px",
            position: "absolute",
            border: "2px solid #fff",
            width: iconWidth || "auto", // or a default value
            height: iconHeight || "auto", // or a default value
          }}
          style={sx}
          onClick={(e) => {
            importData(false);
...
export default ImageCropperCrm;
