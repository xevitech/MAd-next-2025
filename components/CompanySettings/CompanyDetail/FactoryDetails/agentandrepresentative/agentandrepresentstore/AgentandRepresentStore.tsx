...
const AgentandRepresentStore = (): any => {
  ...
  const getCompanyData = async () => {
    ...
    try {
      ...
      const fetchedData = resp?.data?.agents_and_representative;
      if (fetchedData) {
        const parsedData = JSON.parse(fetchedData);
        if (Array.isArray(parsedData?.store) && parsedData.store.length > 0) {
          parsedData.store.forEach((storeEntry) => {
            if (
              Array.isArray(storeEntry.agent_data) &&
              storeEntry.agent_data.length > 0
            ) {
              setData(storeEntry.agent_data);
              setFormBlocks(storeEntry.agent_data);
      ...
      const agent_data = value.map(
        ({ formBlocks, additionalAddresses, ...rest }) => {
          return {
            ...
            additionalAddresses: additionalAddresses.filter(
              (address) => address !== null && address.trim() !== ""
            ),
          };
        }
                          ...
                          <Grid item xs={12}>
                            <DataRowHere className="editview">
                              <Grid
                                ...
                                spacing={1}
                                sx={{
                                  alignItems: formik.errors.forms?.[index]
                                    ?.storeName
                                    ? "baseline"
                                    : "center",
                                }}
...
export default AgentandRepresentStore;
