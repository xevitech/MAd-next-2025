...
export default function DistributorQaQc({ type, type_key }) {
  ...
  const agent_respresent_qaqc = companyDetails?.qaqc_rnd?.[type_key] || null;
  ...
  try {
    ...
    testingImageUrl = agent_image?.[type].values?.testing_certificate;
  } catch (error) {}
  ...
  try {
    ...
    first_pass_yield = agent_list?.[type].values.first_pass_yield;
    certificate_level_quality =
      agent_list?.[type].values.certificate_level_quality;
    no_of_staff = agent_list?.[type].values.no_of_staff;
  } catch (error) {}
  ...
  let formik: any = useFormik({
    initialValues: {
      testing_certificate: agent_respresent_qaqc
        ? JSON.parse(agent_respresent_qaqc)?.[type]?.values?.testing_certificate
        : [],
