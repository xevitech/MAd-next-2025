...
export default function WholesalerQaQc({}) {
  ...
  const agent_respresent_qaqc =
    companyDetails?.qaqc_rnd?.wholesalers_qaqc || null;
  ...
  try {
    ...
    testingImageUrl = agent_image?.Wholesalers.values?.testing_certificate;
  } catch (error) {}
  ...
  try {
    ...
    inspecting_parties = agent_list.Wholesalers.values.inspecting_parties;
    first_pass_yield = agent_list.Wholesalers.values.first_pass_yield;
    certificate_level_quality =
      agent_list.Wholesalers.values.certificate_level_quality;
    no_of_staff = agent_list.Wholesalers.values.no_of_staff;
  } catch (error) {}
  ...
  let formik: any = useFormik({
    initialValues: {
      testing_certificate: agent_respresent_qaqc
        ? JSON.parse(agent_respresent_qaqc)?.Wholesalers?.values
            ?.testing_certificate
        : [],
                        ...
                        <Grid item xs={12} sm={12} md={6}>
                          <DataRowHere>
                            <Grid container spacing={1}>
                              ...
                              <Grid item xs={12} sm={12} md={8}>
                              <DataRowValue>
  <Typography sx={{ display: 'flex', flexWrap: 'wrap', gap: inspecting_parties?.length > 1 ? "7px" : "0" }}>
    {Array.isArray(inspecting_parties) 
      ? inspecting_parties.map((party, index) => (
          <span key={index}>
            {party}
          </span>
        ))
