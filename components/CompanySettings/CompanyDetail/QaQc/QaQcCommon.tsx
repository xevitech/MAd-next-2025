...
import { AstricksMark, ButtonModeHere, CompanyFacilityData, CompanyFacilityInnContainerQAQCnRND, DataRowHere, DataRowTitle, DataRowValue, EditBrowseIcon, EditBrowseText, EditModeBoxContainer, EditSaveIcons, EditSaveIcons1, EditUpImagesStack, SavebtnBox, SaveTextWithIcon, SelectedEditImg, SelectedEditSection, SubHeadingPage, TypographyTitle, UpImageName, UploadImageCol, UploadImagesRow } from "../FactoryDetails/style";
import { CancelTextWithIcon } from "../Certificates/style";
export default function QaQcCommon({type}) {
  ...
  let agent_respresent_qaqc 
if(type=="Distributor"){
  agent_respresent_qaqc =
  companyDetails?.qaqc_rnd?.distributor_qaqc || null;
}
else if(type=='Reseller'){
  agent_respresent_qaqc =
  companyDetails?.qaqc_rnd?.reseller_qaqc || null;
}
...
try {
  ...
  inspecting_parties = agent_list?.[type]?.values.inspecting_parties;
