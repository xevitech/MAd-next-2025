...
const ProductUseCaseComponent = () => {
  ...
  const columns = [
    ...
    //   headerName: "Related Applications",
    //   flex: 1,
    //   minWidth: 100,
    //   sortable: false,
    //   filterable: false,
    ...
    //         <div>
    //           <SellerCatgoryValue>
    //             {cellValues?.row?.related_applications}
    //           </SellerCatgoryValue>
    //         </div>
    //       </>
    //     );
      ...
      renderCell: (cellValues) => {
        return (
          <>
            <div>
              <SellerCatgoryValue>
                {cellValues?.row?.related_products}
              </SellerCatgoryValue>
            </div>
          </>
        );
        ...
        return (
          <>
            ...
            >
                ...
                >
                  <Image
                    onClick={async (e) => {
                      ...
                      setUseCaseId([cellValues?.id]);
                      setUseCaseName(cellValues?.row?.name);
                    }}
                    height={16}
                    width={15}
              ...
              {(role == "seller" ||
                (role == "subuser" &&
                  permissions?.product_use_cases.delete == true)) && (
                <LightTooltip
                  arrow
                  disableInteractive
                  title="Delete"
  ...
  const validation = Yup.object().shape({
    productname: Yup.string().required("Please enter use case name"),
  ...
  const formik: any = useFormik({
    ...
    onSubmit: async (values, { resetForm }) => {
      ...
      let payload = {
        name: productname,
        related_applications: category.join(","),
        related_products: useCase.join(","),
        keywords: keywords.join(","),
        description: description,
        type: "product_use_cases",
      };
      setLoading(true);
  ...
  const DeleteActionHandler = async () => {
    setLoading(true);
    const response = await apiClient("use_cases/delete", "post", {
  ...
  const EditActionHandler = async (e) => {
    setLoading(true);
    const response = await apiClient("use_cases/update", "post", {
      body: {
        id: useCaseId,
        name: useCaseName,
      },
      ...
      {loading ? (
        ...
        <>
            ...
            >
              <Grid container>
                <Grid item xs={12} sm={12} md={3}>
                  ...
                  >
                    <label>Use Cases Name</label>
                    <CustomTextField
                      name="Enter use cases name"
          ...
          <Box>
            {(role == "seller" ||
              (role == "subuser" &&
                permissions?.product_use_cases.add == true)) && (
              <Button
...
export default ProductUseCaseComponent;
