...
import { AddButtonEnquiry, AddSpecification, EnquiryTable, EnquiryTableOuter } from "../style";
...
function EnquiryPopUp({
  ...
  const SaveSpecifications = async (index) => {
    ...
    let payloads = {
      ...
      quantity: quantity,
      price: relatedProductSpecifications?.[index]?.unit_price
        ?.replaceAll("$", "")
        .replaceAll(".00", "")
        .replaceAll(" ", ""),
      type: product_detail?.product_type,
      category_name: product_detail?.category_name,
      country_name: product_detail?.country_name,
      product_datetime: product_detail?.product_datetime,
    ...
    if (response.status == 200 || response.status == true) {
      ...
      dispatch(
        createHistory({
          ...
          name: "Enquiry",
          type: "enquiry",
          message: `<span>Product Added - </span><b>${value?.length > 0 && value?.map((ele) => ele?.value).join('|')}</b> configuration of the product <b>${product_detail?.name}</b> has been added into the enquiry `,
        })
      );
  ...
  return (
    <EnquiryTableOuter>
                        ...
                        <TableCell align="left" key={row.id}>
                          {v.edit ? (
                            <FormControl fullWidth>
                              {/* <InputLabel size="small" id={row.id}>
                              {row.name}
                            </InputLabel> */}
                              <Select
                                sx={{
                    ...
                    <TableCell align="center">
                      {v.edit ? (
                        <Tooltip title="Save" arrow>
                          <SaveIcon
                            sx={{ cursor: "pointer", color: "#24a53b !important", fontSize: '20px !important' }}
      ...
      <AddButtonEnquiry>
        <AddSpecification className="AddSpecenquiry" variant="contained" disabled={buttonDisable} onClick={() => AddSpecifications()}>
