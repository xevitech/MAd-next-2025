...
const BusinessFilter = () => {
  ...
  const filterValue: any = router?.query?.business_type
    ? router.query.business_type.split(",")
    : [];
  ...
  React.useEffect(() => {
    if (checkedBusiness.length === 0 && filterValue.length > 0) {
      setCheckedBusiness(filterValue);
    }
    if (filterValue.length === 0 && checkedBusiness.length > 0) {
      setCheckedBusiness(filterValue);
    }
  }, [filterValue]);
