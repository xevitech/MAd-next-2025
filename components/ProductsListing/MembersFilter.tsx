...
export default function MembersFilter() {
  ...
  const filterValue: any = router?.query?.member_type
    ? router.query.member_type
    : [];

  const addQueryParams = (id) => {
    if (id.length === 0) {
      delete router.query.member_type;
      router.push(router);
      return;
    }
    setFilterMember(id);
    router.query.member_type = id;
    router.push(router);
  };

  return (
    <>
      {filterCategory?.member_type && (
        <MainPageFilterBox>
            ...
            <Collapse in={expanded}>
              <nav aria-label="main mailbox folders">
                ...
                >
                  <ListItem
                    className={
                      filterValue == "verified" && "selected-filter-main-page"
                    }
                    disablePadding
                    onClick={(e) => addQueryParams("verified")}
                  >
                    <ListItemButton>
                      ...
                      <ListItemText
                        primary="Verified Member"
                        style={{
                          color: filterValue == "verified" ? "#D7282F" : "",
                        }}
                      />
                    </ListItemButton>
                  </ListItem>
                  <ListItem
                    className={
                      filterValue == "Unverified" && "selected-filter-main-page"
                    }
                    disablePadding
                    onClick={(e) => addQueryParams("Unverified")}
                  >
                    <ListItemButton>
                      <ListItemIcon className={ProductModule.listimg}>
                        <Image
                      ...
                      <ListItemText
                        primary="Unverified Member"
                        style={{
                          color: filterValue == "Unverified" ? "#D7282F" : "",
                        }}
                      />
                    </ListItemButton>
                  </ListItem>
