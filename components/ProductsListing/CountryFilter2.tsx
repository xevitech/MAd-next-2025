...
import { CategoryBox, CategoryShowLessSpanFilter, MoreLessButton, ProductListWithCheckBox } from "./style";
import KeyboardArrowDownIcon from '@mui/icons-material/KeyboardArrowDown';
import KeyboardArrowUpIcon from '@mui/icons-material/KeyboardArrowUp';
import { fetchFilteredProducts, handleManufactureFilter } from "@/hooks/reducers/manufactureProductsReducer";
...
const addQueryParams = (c) => {
  // console.log("Incoming ID array:", c);

  if (c.length === 0) {
    setCheckedCountry([]);
    delete router.query.country_name; // Remove the query parameter completely
    router.push(router); // Update the router
    dispatch(fetchFilteredProducts({ c: [] }));  
    return; // Exit the function early
  }

  setCheckedCountry(c);
  console.log("Updated Checked country_name:", c);

  router.query.country_name = c.join(","); // Concatenate with commas
  console.log("Router Query country_name:", router.query.country_name); // Log the router query

  router.push(router); // Update the router with the new query
  dispatch(fetchFilteredProducts({ c })); // Dispatch the action with the updated country list
};
// const dispatch = useAppDispatch();
...
const debouncedAddQueryParams = debounce((checkedCategoryArray) => {
  ...
  React.useEffect(() => {
    if (checkedCountry.length === 0 && filterValue.length > 0) {
      setCheckedCountry(filterValue);
    }
    if (filterValue.length === 0 && checkedCountry.length > 0) {
      setCheckedCountry(filterValue);
    }
    if (checkedCountry.join() != filterValue.join()) {
      setCheckedCountry(filterValue);
    }
  }, [filterValue]);

  React.useEffect(() => {
    const filterValue = router.query.country_name ? router.query.country_name.split(",") : []; // Get current businesses from router
    if (Array.isArray(filterValue)) {
      setCheckedCountry(filterValue); // Set checked businesses from router query
    }
  }, [router.query.country_name]); // Run when query parameters change
                ...
                <ProductListWithCheckBox>
                              ...
                              onChange={(e: any) => {
                                  if (e.target.checked) {
                                      ...
                                      setCheckedCountry((prev) => {
                                          ...
                                          // addQueryParams(nc); // Call addQueryParams with the new state
                                          return nc; // Return the new state
                                      });
                                  } else {
                                      // Remove the ID from checkedBusiness
                                      setCheckedCountry((prev) => {
                                          const nc = prev.filter((item) => item !== element.name);
                                          debouncedAddQueryParams(nc);
                                          // addQueryParams(nc); // Call addQueryParams with the new state
                                          return nc; // Return the new state
                                      });
                  ...
                  {!hideViewMore && (
                    <MoreLessButton>
                      ...
                      >
                        {viewMore ? <CategoryShowLessSpanFilter>Less <KeyboardArrowUpIcon /></CategoryShowLessSpanFilter> : <CategoryShowLessSpanFilter>More <KeyboardArrowDownIcon /></CategoryShowLessSpanFilter>}
