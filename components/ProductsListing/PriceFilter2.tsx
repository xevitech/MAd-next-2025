...
const PriceFilter = () => {
  ...
  const [sliderValue, setSliderValue] = React.useState([500, 50000]);
  ...
  let values = router?.query?.range ? router?.query?.range.split(",") : [];
  const handleApply = () => {
  ...
  const addQueryParams = (id) => {
    if (id.length === 0) {
      // dispatch(fetchFilteredProducts({price:[]}));
      delete router.query.range;
      router.push(router);
      return;
    }
    router.query.range = id.join(",");
    router.push(router);
  };
