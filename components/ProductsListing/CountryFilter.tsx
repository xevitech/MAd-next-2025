...
const CountryFilter = () => {
  ...
  const filterValue: any = router?.query?.country
  ...
  const addQueryParams = (id) => {
    ...
    router.query.country = id.join(",");
    router.push(router);
  };

  React.useEffect(() => {
    if (check) dispatch(handleProductFilter());
  }, [check]);
                        ...
                        <MylistItem key={`${element.name}+${i}`} disablePadding>
                          <ListItemButton role={undefined} dense>
                            <FormControlLabel
                              ...
                              checked={checkedCountry.includes(element.code)}
                              name={element.name}
                              onChange={(e: any) => {
                                setCheck(true);                                
                                if (e.target.checked) {
                                  ...
                                  let filter = checkedCountry.filter(
                                    (item) => item !== element.code
                                  );
