...
type CountryFilterProps = {
  onChange: (selectedFilters: any[]) => void; // Function to handle selected filters
  checkedValues: string[]; // Array of currently selected country values
};

const CountryFilter4: React.FC<CountryFilterProps> = ({
  onChange,
  checkedValues,
}) => {
  ...
  const [checkedCountry, setCheckedCountry] =
    React.useState<string[]>(checkedValues);
  ...
  const debouncedAddQueryParams = debounce((checkedCategoryArray) => {
    onChange(checkedCategoryArray);
  }, 300);

  const handleCountry = (event: string) => {
    if (event === "") {
  ...
  React.useEffect(() => {
    setCheckedCountry(checkedValues);
  }, [checkedValues]);
    ...
    <Box className="productnavcheck">
      <CategoryBox sx={{ position: "relative" }}>
        ...
        <Stack
          ...
          position="relative"
        >
          <Categorytitle variant="h6" className={ProductModule.filter_head}>Country</Categorytitle>
          <ExpandMore
            expand={expanded}
                ...
                {countryList.map((element, i) => (
                  <MylistItem key={`${element.name}+${i}`} disablePadding>
                    <ListItemButton role={undefined} dense>
                      <FormControlLabel
                        control={<Checkbox />}
                        label={<ListItemText primary={element.name} />}
                        checked={checkedCountry.includes(element.name)}
                        name={element.name}
                        onChange={(e: any) => {
                          if (e.target.checked) {
                            setCheckedCountry((prev) => {
                              const newChecked = [...prev, element.name];
                              debouncedAddQueryParams(newChecked);
                              return newChecked;
                            });
                          } else {
                            setCheckedCountry((prev) => {
                              const newChecked = prev.filter(
