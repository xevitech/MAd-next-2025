...
export const BusinessTypeForBuyer = (props: any) => {
  const { defaultData, changeMode, toggleMode, resetModes, mode, handleClose } =
    props;
  ...
  const defaultValues = defaultData?.business_type_user;
  ...
  useEffect(() => {
    ...
    setSelectedOptions(
      defaultTypes?.map((v) => ({
        ...
        toggle: v.toggle,
        tooltip: user_info?.type == "buyer" ? v.tooltip : v.value,
      }))
    );
  }, [defaultValues]);
  ...
  const BusinessTypeHandler = (event: any, order, parent = "") => {
    ...
    if (order == "child") {
      ...
      if (index !== -1) {  
          ...
          let updatedItem = { ...updatedTypes[index] };
          
          if (event.checked) {
              if (Array.isArray(updatedItem.child)) {
                  updatedItem.child = [...updatedItem.child, event.value];  
              } else {
                  updatedItem.child = [event.value];
              }
          } else {
              if (Array.isArray(updatedItem.child)) {
                  updatedItem.child = updatedItem.child.filter((v) => v != event.value);
              }
          }
  
          updatedTypes[index] = updatedItem; 
          setSelectedOptions(updatedTypes);  
      }
      return;
  }
  };
  ...
  function addTooltip(array) {
    return array.map((obj) => {
      const tooltips = [];
      const gridOne = GridOneBuyerOptions?.find(
        (option) => option.name === obj.name
      );
      const gridTwo = GridTwoBuyerOptions?.find(
        (option) => option.name === obj.name
      );
      if (gridOne) {
  ...
  const SaveValues = async () => {
    ...
    let dataTosend = {
      business_type_user: JSON.stringify(
  ...
  return (
    <ContentInnerContainer breakPoints={breakPoints}>
      ...
      <OuterContainer style={{ marginTop: "0px" }} sx={CheckBoxStyle}>
        {mode == "edit" ? (
          <>
            <Grid container spacing={1}>
              <Grid item xs={12} sm={6} md={6} lg={6}>
                <Grid container spacing={1}>
                  {GridOneBuyerOptions.map((value, index) => {
              ...
              <Grid item xs={12} sm={6} md={6} lg={6}>
                <Grid container spacing={1}>
                  {GridTwoBuyerOptions.map((value, index) => {
                    const defaultChecked = selectedOptions?.find(
                      (v) => v.name == value.name
                    );
                    const defaultToolt = selectedOptions?.find(
                      (v) => v.name == value.name
                    );
