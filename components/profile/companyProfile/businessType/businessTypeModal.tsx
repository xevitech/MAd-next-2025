...
import { SelectedBusinessBtnOutline, SaveLinkModal } from "./styles";
...
export const BusinessTypeModal = (props: any) => {
  ...
  const BusinessTypeHandler = (event: any, order, parent = "") => {
    ...
    if (order === "child") {
      const index = selectedTypes.findIndex(v => v.name === parent);
      const updatedTypes = selectedTypes.map((item, i) => 
        i === index
          ? {
              ...item,
              child: event.checked
                ? [...(item.child || []), event.value]
                : (item.child || []).filter(v => v !== event.value)
            }
          : item
      );
  ...
  const SaveValues = async () => {
    ...
    if (response?.status === 200 || response?.status === true) {
      ...
      response.message ===
        "You can change your business type only once a month."
    ) {
      ...
      setOtpMessage({
        status: "error",
        message: `You are unable to change your business type right now. Since you last changed it on ${startDate}, you must wait ${daysLeft} days before changing your business type.`,
      });
    } else if (
      response?.status === false &&
      response.message === "You cannot change more than 3 times in a year."
    ) {
      const startDate = moment(response?.date).format("DD-MMMM-YYYY");
      const daysLeft = moment(response?.date)
        .add(365, "days")
