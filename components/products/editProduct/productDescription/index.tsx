...
import { FileUploadProductDescription } from "./fileUploader";
...
import KeywordRatingPage from "./KeywordRating";
...
export const ProductDescription = ({
  ...
  const SignupSchema = Yup.object().shape({
    ...
    upload_files: Yup.array()
      .required("Please upload datasheet, drawing, catalog etc")
      .min(1, "Please upload datasheet, drawing, catalog etc"),
  ...
  const descriptionFormik: any = useFormik({
    ...
    onSubmit: async (values) => {
      ...
      if (selectedKeywords?.length > 0) {
        const keywords = selectedKeywords?.map((item) => item?.name);
        formData.append("meta_keyword", keywords.join(","));
      }
  ...
  useEffect(() => {
    ...
    //   descriptionFormik.setFieldValue(
    //     "selectedKeywords",
    //     JSON.parse(localStorage.getItem("product_keyword")) ?? []
    //   );
    // }, [JSON.stringify(JSON.parse(localStorage.getItem("product_keyword")))]);

    useEffect(() => {
      const updateKeywords = () => {
        const storedKeywords = localStorage.getItem("product_keyword");
        descriptionFormik.setFieldValue("selectedKeywords", storedKeywords ? JSON.parse(storedKeywords) : []);
      };
    
      updateKeywords(); // Initial load
  ...
  useEffect(() => {
    const { name, description, meta_keyword } = productDetail;
    if (name && description?.length > 0 && !isMount) {
      setInformationBlock({ disable: false, expanded: true });
      setCompletedFields((prev) => ({ ...prev, description: true }));
      setIsMount(true);
    }
  ...
  const handleDeleteApplication = async (option) =>{

    const toDelete = descriptionFormik?.values?.product_applications.find(
      (v: any) => v?.name?.toLowerCase() == option?.toLowerCase()
    );
    setIsDisabledProd(true)
    try {
      const response = await apiClient(
        "product-application/application/remove",
        "post",
        {
          body: { product_id: productId, id: toDelete?.id },
        }
      );
      if(response?.status){
        setIsDisabledProd(false)
  ...
  const handleDeleteUseCase = async (option) => {
    const toDelete = descriptionFormik?.values?.product_use_cases.find(
      (v: any) => v?.name?.toLowerCase() == option?.toLowerCase()
    );
    setIsDisabledCase(true)
    try {
      const response = await apiClient(
        "product-application/usecase/remove",
        "post",
        {
          body: { product_id: productId, id: toDelete?.id },
        }
    ...
    setIsDisabledCase(false)
    const value = descriptionFormik?.values?.product_use_cases.filter(
      (v: any) => v?.name != option
    );

    descriptionFormik.setFieldValue("product_use_cases", value);
  };
          ...
          <Grid container spacing={2}>
            <Grid item xl={12} lg={12} md={12} xs={12}>
              ...
              >
                ...
                <TextFieldHelperText sx={{ minHeight: "0px !important" }}>
                  Maximum character{" "}
                                ...
                                >
                                  <AttributeCheckChipBox>
                                    ...
                                    <ManualKeywordChip
                                      ...
                                      {...getTagProps({ index })}
                                      onDelete={(e) => {
                                        handleDeleteApplication(option)
                                      }}
                                    />
                                  </AttributeCheckChipBox>
                                </LightTooltip>
            ...
            <Grid item xl={12} lg={12} md={12} xs={12}>
              ...
              >
                ...
                {descriptionFormik?.errors?.upload_files && (
                  <FormHelperText>
                    {"Please upload datasheet, drawing, catalog."}
                  </FormHelperText>
                )}
                {docError?.error && (
