...
export const Calculator = ({
    ...
    if (operation === "e") {
      ...
      if(operation === "e" && lastSecondItem.type == 'numeric'){
        const updatedData = [
          ...
          { type: "numeric", view: `${lastSecondItem.view}e`, value: (+lastSecondItem?.value * Math.E) }, // Update the last item with 'e' value
        ];
        return updatedData;
      }
      const updatedData = [
  ...
  //         const updatedCurrent = current.type === "trig"
  ...
  //         }
  //       } else {
  //         // For non-numeric values, add them to the accumulator
  //         acc.push({ ...current });
  //       }
  //       return acc;
  //     }, []);
  ...
  const mergeNumericValues = useMemo(() => {
    return (equation) => {
      return equation?.reduce((acc, current) => {
        if(current.type === "numeric" && acc[acc.length - 1]?.view?.includes("e")){
  ...
  useEffect(() => {
    ...
    if (type !== 1) {
      if (calculatorType === "Group") {
        if (!groupFinalState || groupFinalState.length === 0) {
          setGroupFinalState([]);
          return;
        }

        const lastItem = groupFinalState[groupFinalState.length - 1];
        const updatedState = mergeNumericValues(groupFinalState);

        updateStateIfChanged(groupFinalState, updatedState, setGroupFinalState);

        if (lastItem?.type === "trignometric") {
          const operation = lastItem.value;
          const updatedData = performCalculation(operation, groupFinalState);
        ...
        const lastItem = equation[equation.length - 1];
        const updatedState = mergeNumericValues(equation);

        updateStateIfChanged(equation, updatedState, setEquation);
        updateStateIfChanged(equation, updatedState, setFinalPayloads);

        if (lastItem?.type === "trignometric") {
          const operation = lastItem.value;
          const updatedData = performCalculation(operation, equation);
          updateStateIfChanged(equation, updatedData, setEquation);
          updateStateIfChanged(equation, updatedData, setFinalPayloads);
        }
      }
    } else {
      if (calculatorType === "Group") {
        ...
        updateStateIfChanged(equation, updatedState, setData);

        if (lastItem?.type === "trignometric") {
          const operation = lastItem.value;
          const updatedData = performCalculation(operation, equation);
          updateStateIfChanged(equation, updatedData, setData);
        }
      } else {
        if (!finalCalculation || finalCalculation.length === 0) {
          setEquation([]);
          return;
        }

        const lastItem = finalCalculation[finalCalculation.length - 1];
        const updatedState = mergeNumericValues(finalCalculation);

        updateStateIfChanged(finalCalculation, updatedState, setEquation);

        if (lastItem?.type === "trignometric") {
          const operation = lastItem.value;
          const updatedData = performCalculation(operation, finalCalculation);
          updateStateIfChanged(finalCalculation, updatedData, setEquation);
        }
      }
    }
  }, [
    equation ? JSON.stringify(equation) : null,
    groupFinalState ? JSON.stringify(groupFinalState) : null,
    finalCalculation ? JSON.stringify(finalCalculation) : null,
    type,
    calculatorType,
    mergeNumericValues,
    performCalculation,
  ...
  //   const secondLastElement = equation?.[equation?.length - 2];
  ...
  //   if (lastElement?.type == "numeric" && leftValues?.length !== 0) {
  ...
  const handleCalculatorKeyClick = async (e) => {
    ...
    if (name == "clear") {
      ...
      const endElementType = endElement?.type;
      if (setEquation) {
        setEquation((prev) => prev?.slice(0, -1));
        setFinalPayloads((prev) => prev?.slice(0, -1));
      }
      if (setGroupFinalState) {
    ...
    if (type !== 1) {
      if (calculatorType === "Group") {
        ...
        setGroupFinalState((prev) => {
          if (
            (prev.length > 0 && prev[prev.length - 1]?.type == "pie" && name != "operand") ||
            (prev.length > 0 && name == "pie" && prev[prev.length - 1]?.type != "operand")
          ) {
            hasError = true;
            return prev;
          }
        ...
        setGroupFinalState((prev) => {
          ...
          if(prev?.length > 0 && prev[prev.length - 1]?.type !== "operand" && (value =="(")){
            hasError = true;
            return prev;
          }
          return prev;
        ...
        setGroupFinalState((rev) => {
          if (
            ...
            rev[rev.length - 1]?.type == "tagInput" && value == '.'){
              hasError = true;
              return rev;
          }
          return rev;
        ...
        //   ) {
        //     hasError = true;
        //     const clone = rev.slice(0, rev.length-1);
        ...
        selectedGroupsData((prev) => {
          // if (prev?.length && prev[prev.length - 1]?.type !== "operand" &&  !allowedOperands.includes(name)) {
          //   toast.error("please enter operator6.");
          ...
          if(value == '.'){
            ...
            if (
              ...
              if (
                ((prev[prev.length - 1]?.value == "(" ||
                  prev[prev.length - 1]?.value == ")" || prev[prev.length - 1]?.value == "π") &&
                  name == "operand") ||
                value == "(" ||
                value == ")" || value == "π"
              ) {
      ...
      if (calculatorType != "Group") {
        let object = { view: value, value, type: name };
        if(name == "pie"){
          object = { view: value, value:"3.14", type: name };
        }
        setData(object);
        // setFinalEquation([value]);
      } else {
        let object = { view: value, value, type: name };
        if(name == "pie"){
          object = { view: value, value:"3.14", type: name };
