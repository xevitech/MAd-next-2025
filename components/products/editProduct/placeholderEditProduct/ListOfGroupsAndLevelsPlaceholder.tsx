...
import PopDesign from "../../editProduct/productInformation/activePassive/active/GroupsAndLevels/detail.module.css";
...
import {
  ...
  SmallOutlineNewBtn,
  TextandButtonGroup,
} from "../productInformation/activePassive/active/GroupsAndLevels/styles";
import PricingCalculator from "../productInformation/activePassive/active/pricingCalculator";
import { CustomSelector } from "../productInformation/activePassive/active/GroupsAndLevels/CustomSelector";
import {
  HeaderCustomSpecs,
  HeaderMainText,
  HeaderSubText,
} from "../productInformation/activePassive/active/customSpecs/styles";
import { GroupsTable } from "../productInformation/activePassive/active/GroupsAndLevels/groupsTable";
import { LevelsTable } from "../productInformation/activePassive/active/GroupsAndLevels/levelsTable";
...
export const ListOfGroupsAndLevelsPlaceholder = (props) => {
          ...
          <>
            <FormControl>
              <FormControlLabel
                ...
                label={
                  <AttributeDependency>
                    {params?.row?.isDepended ? "Depends On" : "Independent"}
                  </AttributeDependency>
                }
              />
            </FormControl>
            ...
            {params?.row?.isDepended && (
              ...
              >
                <MenuItem disabled>Select Specification</MenuItem>
  ...
  const handleChange = (event, rowData) => {
    ...
    const isDepended = rows?.map((item) => {
      if (item?.id == rowData?.id) {
        return { ...item, isDepended: event.target.checked };
      }
      return item;
    });
    setRows(isDepended);
  ...
  const handleLevelSubmission = () => {
    ...
    if (!selectedOrNot) {
      toast?.error(
        "Please select atleast one specification to generate levels."
      );
      return;
    }
  ...
  return (
    <>
      ...
      <div>
        ...
        {specsData?.is_group == "no" && specsData?.payload?.length > 0 && (
