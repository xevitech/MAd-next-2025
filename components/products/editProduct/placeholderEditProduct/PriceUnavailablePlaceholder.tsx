...
import { setShowByOrderAddBrand } from "@/hooks/ProductReducers";
...
export const PriceUnavailable = ({
  ...
  const { brand_id } = byOrderFormik.values;
  ...
  useEffect(() => {
    ...
    if (product_type === "simple") {
      ...
      if (availability === "by_order") {
        HandlePercentage("brand_id", brand_id ? manufacturerBrand : 0);
      }
    } else {
      const { manufacturerBrand } =
  ...
  const onChangeHandler = (e) => {
    ...
    if (regex.test(e.target.value)) {
      ...
      let id =
        brandsData?.find(
          (v) => v.view.toLowerCase() == `${e.target.value}`.toLowerCase()
        )?.value ?? "";
      if (id) {
        setBrand(id);
  ...
  const onSubmitHandler = () => {
    ...
    AddByOrderBrand(setLoading, customBrand, setError, setErrorText);
  };
