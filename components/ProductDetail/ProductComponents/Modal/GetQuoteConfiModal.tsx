...
import { QuotationFileUpload } from "@/components/common/uploadFile/QuotationFileUpload";
...
const GetQuoteConfigModal = ({ toggleFlyout, type }) => {
  const dispatch = useDispatch();
  const { productConfig }: any = useSelector(
    (state: any) => state.productDetail
  );
  const [options, setOptions] = useState<any>([]);
  const { variation_options, breadcrumbs } = useSelector(
  ...
  const filterVariations = async () => {
    ...
    try {
      ...
      let response = await apiClient(
        `front/matrix/list?search=${sanitizedSearchData}&product_id=${id}&product_type=configured&currency=${currencyId}`,
        "GET"
      );
  ...
  const rows = useMemo(() => {
    return filterData?.map((item, index) => {
      ...
      return {
        ...
        // page === 1
        //   ? index + 1
        //   : index + 1 + (page - 1) * matrixResponse?.data?.length,
        indexValue: index,
        id: item.matrix_id,
  ...
  const [skeltonGetData, setGetSkeltonData] = useState(false);
  const getVariationList = async () => {
    setGetSkeltonData(true);
    try {
      const response = await fetch(
        `${BASE_URL}/front/temp_quote/matrix/list?product_id=${id}`,
        {
          method: "get",
          headers: {
  ...
  const getQuote = async (newVariation) => {
    ...
    formData.append("unique_id", id.toString());
  ...
  const handlePaginatonChange = async (
    ...
    const sanitizedSearchData = searchData
      ...
      .replace(/,{2,}/g, ",");
    let response = await apiClient(
      `front/matrix/list?search=${sanitizedSearchData}&product_id=${id}&product_type=configured&page=${value}`,
      "GET"
    );
    if (response.status === 200) {
      const newQuantity = response?.data.map((da) => {
  ...
  const getSelectedVariations = () => {
    return rows
      .filter((row) => selectionModel.has(row.id))
                      ...
                      {isButtonEnabled && (
                        <>
                          ...
                          <FlyOutTableButton className="configFlyout">
                            ...
                            <FlyoutBtn
                              className={
                                idArray?.length === 0 ? "ConfigQuote" : ""
                              }
                              disabled={
                                idArray?.length === 0 || !isButtonEnabled
                              }
                              onClick={() => {
...
export default GetQuoteConfigModal;
