...
const QuoteModal = ({ open, handleClose }) => {
  ...
  const fetchSingleProductDetails = async () => {
    ...
    if (response.status === 200) {
      setCustomiseRequestData(response?.data?.category_lists);
    }
  };
  useEffect(() => {
  ...
  if (Array.isArray(relatedIds)) {
    ...
    // Handle null/undefined or throw error if needed
    relatedProductIds = [];
  }
  ...
  const finalPrice: any =
    customize_request?.price && customize_request?.quantity
      ? customize_request?.price * customize_request?.quantity
      : 0;
  const final_price: any =
    quotedetails?.unit_price * customize_request?.quantity;
  const SubmitQuotation = async (values) => {
    ...
    if (quotedetails.product_type == "simple") {
       if (values?.customize_request?.quantity == "") {
        ...
        return;
      } else if (values?.customize_request?.quantity == 0) {
    ...
    formData.append("unit_price", customize_request?.total_price);
    ...
    formData.append(
      "price",
      quotedetails?.unit_price || customize_request?.price
    );
        ...
        <form onSubmit={formik.handleSubmit}>
          ...
          >
            ...
            <DrawerBody>
                      ...
                      <UploadImgBox sx={{ margin: "4px 0 0 0" }}>
                        ...
                        >
                          <FileUpload
                            ...
                            updateFiles={(e) => { 
                                const findType = e[e.length-1]?.name.indexOf(".");
                                if (e[e.length-1]?.name.slice(findType+1) != "pdf" 
                                && e[e.length-1]?.name.slice(findType+1) !="doc" 
                                && e[e.length-1]?.name.slice(findType+1) != "xls" 
                                && e[e.length-1]?.name.slice(findType+1) !="docx" 
                                && e[e.length-1]?.name.slice(findType+1) != "xlsx"){
              ...
              {value === 0 && quotedetails?.hide_price === 1 && (
                <SummaryOuter>
                  <Box
                    sx={{
                      width: "300px",
